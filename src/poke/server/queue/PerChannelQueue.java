...
import com.lifeforce.RemoteConnection;
					...
					if (msg instanceof Request) {
						...
						if (rsc == null) {
							...
							if(rsc instanceof JobResource) {
								...
								if(reply.getHeader().getReplyMsg().equals(JobResource.FORWARD)) {
									RemoteConnection remoteCon = new RemoteConnection(NetworkManager.getInstance().getNextNodeIP(),
											NetworkManager.getInstance().getNextNodePort(), sq);
									logger.info("Forwarding request to " + NetworkManager.getInstance().getNextNodeIP() + ":" + NetworkManager.getInstance().getNextNodePort());
									remoteCon.sendMessage(reply);
								} else {
									logger.info("Returning response to caller");
									sq.enqueueResponse(reply, null);
								}
								
							} else if (rsc instanceof ForwardResource) {
								logger.info("Forwarding write request to " + NetworkManager.getInstance().getNextNodeIP() + ":" + NetworkManager.getInstance().getNextNodePort());
								RemoteConnection remoteCon = new RemoteConnection(NetworkManager.getInstance().getNextNodeIP(),
										NetworkManager.getInstance().getNextNodePort(), sq);
								remoteCon.sendMessage(req);
							} 
						}
					}
				} catch (InterruptedException ie) {
