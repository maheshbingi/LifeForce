...
//		mhb.addPath(rpb);
	...
	public void concludeWith(boolean success, Integer leaderID) {
		if (success) {
			...
			// Inter cluster implementation
			if(ClusterDBServiceImplementation.isInterClusterImplementation()) {
				//set details of the newly selected leader in cluster shared db for handling intercluster communication
				if(this.leaderNode == conf.getNodeId()) {
					ClusterMapperManager.setClusterDetails(conf.getHost(), conf.getPort());
				}
			}
		}
