...
public class HeartMonitor {
	...
	public boolean startHeartbeat() {
		...
		try {
			...
			SocketAddress socka = ch.localAddress();
			InetSocketAddress isa = (InetSocketAddress) socka;
			
			String ip = isa.getAddress().getHostAddress();
			
			System.out.println("$$$$$$$$$$$ In NODE_JOIN $$ IP_Address is SET TO: "+ip);
			mhb.setIpAddress(ip);
			mhb.setPort(HeartbeatManager.getServerConf().getPort());
	...
	public static class MonitorClosedListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			...
			System.out.println("@@@@@@@@@@@@@@@@ monitor.toNodeId @@@@@@@@@@@@@@ "+monitor.toNodeId);
			System.out.println("@@@@@@@@@@@@@@@@ electionManager.whoIsTheLeader @@@@@@@@@@@@@@ "+electionManager.whoIsTheLeader());
			
			if((electionManager.whoIsTheLeader() != null) && (monitor.toNodeId == electionManager.whoIsTheLeader())){
				System.out.println("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX LEADER IS DEAD XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
				electionManager.declareLeaderDead();
			}
			
			ConcurrentHashMap<Channel, HeartbeatData> aa = HeartbeatManager.getInstance().getOutgoingHB();
			Channel ch = aa.keySet().iterator().next();
			ch.writeAndFlush(generateDeadSignal());
			System.out.println("@@@@@@@@@@@@@@@@------ DEAD SIGNAL SENT -------@@@@@@@@@@@@@@@@");

		}

		private Management generateDeadSignal() {
			Network.Builder n = Network.newBuilder();
			NetworkRecoveryData.Builder nrData = NetworkRecoveryData.newBuilder();
			HeartbeatData hbData = NetworkManager.getInstance().getSelfInfo();
			nrData.setNodeId(hbData.getNodeId());
			nrData.setHost(hbData.getHost());
			nrData.setPort(hbData.getPort());
			nrData.setMgmtport(hbData.getMgmtport());
			nrData.setIsStorageNode(JobManager.getInstance().isStorageNode());
			
			// 'N' allows us to track the connection restarts and to provide
			// uniqueness
			n.setFromNodeId(monitor.iamNode);
			n.setToNodeId(monitor.toNodeId);
			n.setNetworkRecoveryData(nrData);
			n.setAction(NetworkAction.NODEDEAD);

			MgmtHeader.Builder mhb = MgmtHeader.newBuilder();
			mhb.setOriginator(monitor.iamNode);
			mhb.setTime(System.currentTimeMillis());
